...
//            $this->Image('fpdf/logoempresa.jpg',5,5,25);
            ...
            $this->Ln(5);
            $this->Cell(190,5,utf8_decode("Listado de vehículos en el parqueadero ordenado por dependencia"), 0, 0, 'C');
            $this->Ln(5);
        }
    ...
    if( odbc_num_rows( $reg ) ) {
        ...
        while ( $res = odbc_fetch_array($reg) ) {
            ...
            if ( $desproce != trim($res[odbc_field_name($reg, 2)]) ) {
                
                if ( trim($res[odbc_field_name($reg, 2)]) == '' ){
                    
                    $pdf->Cell($tamcelda,4,"Sin proceso ", 0, 0, 'J');
                    
                } else {
                    
                    $pdf->Cell($tamcelda,4,$res[odbc_field_name($reg, 2)], 0, 0, 'J');
                    
                }
                $desproce = trim($res[odbc_field_name($reg, 2)]);
                $pdf->Ln();
            }
            
            $pdf->Cell(5,4,"", 1, 0, 'J');
            $pdf->Cell(12,4,$res[odbc_field_name($reg, 3)], 0, 0, 'J');
            $pdf->Cell($tamcelda-17,4,$res[odbc_field_name($reg, 4)], 0, 0, 'J');
            $pdf->Ln();
