...
if ( $parqueadero > 0 ) {
    ...
    if ( $parqueadero == 1 ) {
        ...
        if ( $numerocajaabiertas > 0 or ( $dirip == '10.25.20.170') ) {
            ?>
            <h1>
                No permite ingresos
            </h1>
            <p class="alert alert-danger">
                ...
                - Usted tiene caja abierta y por lo tanto no se le permite realizar ingresos de vehiculos en el sistema.<br/>
                - Está trabajando desde el equipo de salidas y no debe registrar entradas <br/>
    ...
    if ( isset ( $_POST['ingresar'] ) ) {
        ...
        $mrcodcons =0;
        if ( strlen($placa) > 4 ) {
            // Consultar informacion de la policia nacional.
            ...
            //Validar si tiene factura realizada desde micomfamiliar solo para parqueadero 3
            if($parqueadero == "3"){
                $urlApi = "https://muf.comfamiliar.com/app_dev.php/api/micomfamiliar/verificarregistroparqueadero/" .$placa ."/" . $tipo ."/". $fecha;
                $responseMicomfamiliar = file_get_contents($urlApi);
                $response = json_decode($responseMicomfamiliar,true);
                
                if($response["data"]){
                    $mrcodcons= $response["data"]["MRCODCONS"];
                    $fecfac = $fecha;
                    $horfac = $hora;
                    $usufac='micomfa';
                }
            ...
            // Validar si tiene ingreso activo del mismo dia.
            $select = " select id 
                from movimiento
                where parqueadero = '$parqueadero' and placa = '$placa' and mrcodcons = 0 and fecing = '$fecha' 
                    and fecfac = '0' and fecsal = '0' and estado = 'A' ";
            $consultar = $parqueo->consulta($select);
            $resultado = odbc_fetch_array($consultar);
            $movimiento = trim($resultado[odbc_field_name($consultar, 1)]);
            if( $movimiento ) {
                $res = odbc_fetch_array($reg);
                $_SESSION['mvtoid'] = $movimiento;
                // SOPORTE 46231 PARA EMPLEADOS NO SE IMPRIME TIQUETE 20200217
                $query = " select v.nombre as persona, v.tipousua, tu.nombre as tipo, v.color, v.tipovehi
                from vehiculos v inner join motivo tu on v.tipousua = tu.criterio and tabla = 'tipousua'
                where v.placa = '%s' and v.estado = 'A' ";
                $setQuery = sprintf($query, $placa);
                $resultQuery = $parqueo->consulta($setQuery);
                $resultadoQueryArray = odbc_fetch_array($resultQuery);
                
                if($parqueadero == '1'){
                    if(empty($resultadoQueryArray)){
                    ?>
                    ...
                    <?php
                        ...
                        $tipoDeUsuario = $resultadoQueryArray[odbc_field_name($resultQuery, 2)];
                        if($tipoDeUsuario !='E'){?>
                            <script type="text/javascript">
                                window.open("web/mvtoingresowimp.php?mvtoid=<?php echo $_SESSION['mvtoid']; ?>","Ingreso","width=500,height=500,menubar=no");
                                window.location.href="app.php?web=mvtoingreso";
        ...
        <div class="row-fluid">
            <div class="span4 letraGrande">
                <form action="" method="POST">
                    ...
                    $parqueo->select($tipos, 'estavehi', '40', 'span12');
