...
$sql = " select distinct v.placa, v.canumdocum, v.nombre, tu.nombre as tipousua, 
    ...
    where v.estado = 'A' ";
$encabezados = "Placa,Identidad,Nombre completo,Dependencia / tipo,Vehiculo,Estado,Col,Fec lim,Aut,Procedencia";
            ...
            $tabla.= "<tr>";
                ...
                $tabla.= "<td>";
                    if ( trim($res[odbc_field_name($reg, 10)]) != "" )
                        $tabla.= ucwords(strtolower($res[odbc_field_name($reg, 9)]));
                    else
