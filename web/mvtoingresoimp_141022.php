...
if ( $mvtoid > 0 ) {
    ...
    if( odbc_num_rows($consulta) ) {
        ...
        $horin = $res[odbc_field_name($consulta, 4)];
        if ( $horin > 99999 ) {
    ...
    $pdf->Ln(2);
    $pdf->MultiCell(72,3,utf8_decode('Comfamiliar Risaralda está suministrando  el derecho a usar el espacio para su vehículo y no se hace responsable por daños ni por hurtos. Este tiquete es sólo un control sobre el acceso y salida de vehículos. La administración no puede garantizar la vigilancia de los vehículos y de sus accesorios por lo tanto cierre bien su vehículo y no deje objetos de valor en él. Este tiquete no es comprobante de recibo del vehículo, ni constituye prueba de depósito ni de contrato alguno.'), 0, 'J');
    $pdf->Ln(2);
    $pdf->MultiCell(72,3,utf8_decode('Señor usuario: antes de retirar su vehículo favor pasar por el punto de pago para cancelar el uso del servicio.'), 0, 'J');
    $pdf->AutoPrint(false);
    $pdf->Output();
