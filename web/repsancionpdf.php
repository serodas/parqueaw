...
//            $this->Image('fpdf/logoempresa.jpg',5,5,25);
            ...
            $this->Ln(5);
            $this->Cell(190,5,utf8_decode("Listado de vehículos en el parqueadero ordenado por placa, con la dependencia"), 0, 0, 'C');
            $this->Ln(5);
        }
    ...
    $procesos = " select distinct p.prcodproce, p.prdesproce, v.placa
        from vehiculos v
        ...
        group by p.prcodproce, p.prdesproce, v.placa
        order by v.placa ";
    ...
    if( odbc_num_rows( $reg ) ) {
        ...
        while ( $res = odbc_fetch_array($reg) ) {
            ...
            if ( trim($res[odbc_field_name($reg, 2)]) == '' ){
                ...
                $pdf->Cell($tamcelda,4,substr($res[odbc_field_name($reg, 2)],0,25), 0, 0, 'J');
