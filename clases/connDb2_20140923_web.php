...
class DB2 {
    ...
    public function minutosAHoraDb($horval, $tiempo) {
        
        $tamhor = strlen($horval);
        if ( $tamhor == 3 ) {
            $hor = substr($horval, 0, 1);
            $min = substr($horval, 1, 2);
        } else {
            $hor = substr($horval, 0, 2);
            $min = substr($horval, 2, 2);
        }
...
//        $horaFinal[1] = 0;
//        $horaFinal[2] = 0;
        $horaFinal = array(0, 0, 0);
        $tamhor = strlen($hora);
        
        switch ($tamhor) {
            case 1:
                $horaFinal = array(0,0,$hora);
                break;
            case 2:
                $horaFinal = array(0,0,$hora);
                break;
            case 3:
                $horaFinal = array(0,substr($hora,0,1),substr($hora,1,2));
                break;
            case 4:
                $horaFinal = array(0,substr($hora,0,2),substr($hora,2,2));
                break;
            case 5:
                $horaFinal = array(substr($hora,0,1),substr($hora,1,2),substr($hora,3,2));
                break;
            case 6:
                $horaFinal = array(substr($hora,0,2),substr($hora,2,2),substr($hora,4,2));
                break;
            default :
                $horaFinal = array(0,0,0);
        }
    ...
    public function restarHoras($fecini, $horini, $fecfin, $horfin ) {
        
        $arrhorini = $this->cortarHora($horini);
        $arrhorfin = $this->cortarHora($horfin);
        ...
        if ( $fecini == $fecfin ) {
            ...
            if ( $horini <= $horfin ) {
                ...
                if ( $arrhorfin[0] == $arrhorini[0] ) {
                    
                    $hora = array(00,$arrhorfin[1] - $arrhorini[1],0);
                    
                } else {
                    
                    
                    if ( $arrhorfin[1] > $arrhorini[1] ) {
                        
                        $hora[0] = $arrhorfin[0] - $arrhorini[0];
                        $hora[1] = $arrhorfin[1] - $arrhorini[1];
                        $hora[2] = 0;
                        
                    } else {
                        
                        $hora[0] = $arrhorfin[0] - $arrhorini[0] - 1;
                        $hora[1] = $arrhorfin[1] + ( 60 - $arrhorini[1]);
                        $hora[2] = 0;
